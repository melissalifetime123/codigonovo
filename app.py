...
if arquivo:
    try:
        ...
        # Referência principal para o Sharpe (usando Global Agg como base neutra)
        ...
        st.markdown(f"<p class='soma-discreta'>Valid. Somatórios: " + " | ".join([f"{p}: {somas[p]:.1f}%" for p in perfis]) + "</p>", unsafe_allow_html=True)
        ...
        with col_res:
            ...
            st.dataframe(res_df.style.format(lambda x: f"{x:.2f}" if "Sharpe" in str(x) else f"{x:.2%}"), use_container_width=True)
        with col_info:
        ...
        with col_stats_L:
            st.write("📋 **Estatísticas Individuais:**")
        ...
        # --- EFICIÊNCIA ---
        st.markdown("---")
        st.subheader("🔍 Análise de Eficiência (Risco x Retorno)")
        col_eff_L, col_eff_R = st.columns(2)

        with col_eff_L:
            st.write("**⚠️ Contribuição de Risco por Ativo:**")
            df_rd = pd.DataFrame(risk_decomp)
            st.dataframe(df_rd.style.format("{:.1% Cory}").background_gradient(cmap='Reds'), use_container_width=True, height=350)
        ...
        with col_eff_R:
            ...
            f_h.update_layout(template="simple_white", xaxis_title="Volatilidade", yaxis_title="Retorno Anualizado",
                              yaxis_tickformat='.1%', xaxis_tickformat='.1%', height=400)
